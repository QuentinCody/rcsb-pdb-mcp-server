...
var _McpAgent = class _McpAgent2 extends DurableObject2 {
  ...
  static serveSSE(path, {
    ...
    return {
      fetch: /* @__PURE__ */ __name(async (request, env2, ctx) => {
        const corsResponse = handleCORS(request, corsOptions);
        if (corsResponse) return corsResponse;
        const url = new URL(request.url);
        const namespace = env2[binding2];
...
var SchemaInferenceEngine = class {
  constructor() {
    this.discoveredEntities = /* @__PURE__ */ new Map();
    this.entityRelationships = /* @__PURE__ */ new Map();
    this.semanticMappings = /* @__PURE__ */ new Map();
    this.typeMappings = /* @__PURE__ */ new Map();
    this.indexSuggestions = /* @__PURE__ */ new Map();
    this.initializeSemanticMappings();
    this.initializeTypeMappings();
  }
  static {
    __name(this, "SchemaInferenceEngine");
  }
  ...
  forceEntityExtraction(data, schemas) {
  ...
  generateEntityName(path, objectKeys) {
    if (path.length > 0) {
  ...
  discoverEntitiesAggressively(obj, path, parentEntityType) {
    ...
    if (obj.edges && Array.isArray(obj.edges)) {
      const nodes = obj.edges.map((edge) => edge.node).filter(Boolean);
      if (nodes.length > 0) {
  ...
  isEntityOrCouldBeEntity(obj) {
    ...
    if (fieldCount >= 3) {
      const hasEntityIndicators = keys.some(
        (key) => ["name", "title", "description", "type", "formula", "sequence", "value"].includes(key.toLowerCase())
      );
  ...
  recordRelationship(fromTable, toTable) {
    if (fromTable === toTable) return;
    const fromRelationships = this.entityRelationships.get(fromTable) || /* @__PURE__ */ new Set();
    const toRelationships = this.entityRelationships.get(toTable) || /* @__PURE__ */ new Set();
  ...
  createSchemasFromEntities(schemas) {
  ...
  extractEntityFields(obj, columnTypes, entityType) {
    const rowData = {};
  ...
  extractNestedScalarFields(obj, parentKey) {
    const fields = {};
  ...
  hasOnlyScalarFields(obj) {
    if (!obj || typeof obj !== "object") return false;
    const values = Object.values(obj);
    return values.length <= 10 && values.every(
      (value) => typeof value !== "object" || value === null || Array.isArray(value) && value.every((v) => typeof v !== "object" || v === null)
    );
  }
  getSemanticColumnName(originalName) {
    const lower = originalName.toLowerCase();
    return this.semanticMappings.get(lower) || originalName;
  }
  inferTypeFromName(columnName) {
  ...
  createJunctionTableSchemas(schemas) {
  ...
  addRelationalConstraints(schemas) {
  ...
  suggestIndexesForTable(tableName, columns) {
    const indexes = [];
  ...
  addIndexSuggestions(schemas) {
    for (const [tableName, schema] of Object.entries(schemas)) {
      const suggestions = this.indexSuggestions.get(tableName);
      if (suggestions && suggestions.length > 0) {
        schema.suggested_indexes = suggestions;
      }
    }
  }
  createSchemaFromPrimitiveOrSimpleArray(data, tableName) {
    const columnTypes = {};
    const sampleData = [];
    if (Array.isArray(data)) {
  ...
  addColumnType(columnTypes, column, type) {
    if (!columnTypes[column]) columnTypes[column] = /* @__PURE__ */ new Set();
    columnTypes[column].add(type);
  }
  resolveColumnTypes(columnTypes, tableName) {
    const columns = {};
  ...
  ensureIdColumn(columns, tableName) {
    if (!columns.id) {
  ...
  sanitizeTableName(name) {
    ...
    let sanitized = name.replace(/[^a-zA-Z0-9_]/g, "_").replace(/_{2,}/g, "_").replace(/^_|_$/g, "").toLowerCase();
  ...
  sanitizeColumnName(name) {
    ...
    let snakeCase = name.replace(/([A-Z])/g, "_$1").toLowerCase().replace(/[^a-zA-Z0-9_]/g, "_").replace(/_{2,}/g, "_").replace(/^_|_$/g, "");
  ...
  addForeignKeyColumn(schema, referencedTableName, parentTableName) {
...
var DataInsertionEngine = class {
  // For generating sequential IDs
  constructor() {
    this.processedEntities = /* @__PURE__ */ new Map();
    this.entityToRowMap = /* @__PURE__ */ new Map();
    this.relationshipData = /* @__PURE__ */ new Map();
    this.semanticMappings = /* @__PURE__ */ new Map();
  ...
  static {
    __name(this, "DataInsertionEngine");
  }
  initializeSemanticMappings() {
    const mappings = {
      // PDB-specific mappings
  ...
  async insertData(data, schemas, sql) {
  ...
  async insertAllEntitiesWithoutForeignKeys(obj, schemas, sql, path = []) {
    if (!obj || typeof obj !== "object") return;
    if (Array.isArray(obj)) {
      for (const item of obj) {
  ...
  async insertEntityRecordWithoutForeignKeys(entity, tableName, schema, sql) {
    const entityMap = this.processedEntities.get(tableName) || /* @__PURE__ */ new Map();
  ...
  async updateForeignKeyRelationships(obj, schemas, sql, path = []) {
    if (!obj || typeof obj !== "object") return;
    if (Array.isArray(obj)) {
      for (const item of obj) {
  ...
  async updateEntityForeignKeys(entity, entityInfo, schema, sql) {
    const updates = [];
    for (const [columnName, columnType] of Object.entries(schema.columns)) {
      if (columnName.endsWith("_id") && columnType.includes("REFERENCES") && !columnType.includes("PRIMARY KEY")) {
  ...
  findEntityBySemanticMapping(entity, baseKey) {
  ...
  findNestedEntityForForeignKey(entity, foreignKeyColumn, baseKey) {
    for (const [key, value] of Object.entries(entity)) {
  ...
  extractEntityFields(entity, schema) {
    const rowData = {};
  ...
  findValueForColumn(entity, columnName) {
  ...
  async insertJunctionTableRecords(data, schemas, sql) {
    await this.discoverAndInsertManyToManyRelationships(data, schemas, sql);
  }
  async discoverAndInsertManyToManyRelationships(obj, schemas, sql, path = []) {
    if (!obj || typeof obj !== "object") return;
    if (Array.isArray(obj)) {
      for (const item of obj) {
  ...
  getJunctionTableName(table1, table22) {
    return [table1, table22].sort().join("_");
  }
  /**
   * Coerces a value to match the expected SQLite column type to prevent SQLITE_MISMATCH errors
   */
  coerceValueToSchemaType(value, expectedType, columnName) {
    try {
      ...
      console.warn(`Type coercion failed for column ${columnName}: ${error3}`);
      return null;
    }
  }
  /**
  ...
  findOriginalKeyWithSemantics(obj, sanitizedKey) {
  ...
  findOriginalKey(obj, sanitizedKey) {
    if (!obj || typeof obj !== "object") return null;
    const keys = Object.keys(obj);
  ...
  async insertSimpleRow(obj, tableName, schema, sql) {
  ...
  mapObjectToSimpleSchema(obj, schema, tableName) {
    const rowData = {};
    for (const columnName of Object.keys(schema.columns)) {
      ...
      if (obj && typeof obj === "object" && !Array.isArray(obj)) {
        ...
        if (originalKey && obj[originalKey] !== void 0) {
...
var PaginationAnalyzer = class {
  static {
    __name(this, "PaginationAnalyzer");
  }
  ...
  static findEdgesArrays(obj, result) {
    if (!obj || typeof obj !== "object") return;
  ...
  static countArrayItems(obj) {
    ...
    for (const value of Object.values(obj)) {
      if (Array.isArray(value)) {
        ...
        count3 += this.countArrayItems(value);
      }
    }
    return count3;
  }
...
// src/do.ts
var JsonToSqlDO = class extends DurableObject3 {
  constructor(ctx, env2) {
    super(ctx, env2);
  ...
  static {
    __name(this, "JsonToSqlDO");
  }
  async processAndStoreJson(jsonData, sourceQuery) {
    try {
      ...
      this.processingStats.lastProcessedAt = /* @__PURE__ */ new Date();
      if (sourceQuery) {
      ...
      return this.createEnhancedError(
        error3 instanceof Error ? error3.message : "Processing failed",
        "processing_error",
        [
        ...
        { error_details: error3 instanceof Error ? error3.stack : void 0 }
      );
    }
  }
  async executeSql(sqlQuery) {
    try {
      ...
      return this.createSqlError(
        sqlQuery,
        error3 instanceof Error ? error3.message : "SQL execution failed",
        this.generateSqlErrorSuggestions(sqlQuery, error3)
      );
    }
  }
  createEnhancedError(message, errorType, suggestions, context2) {
    return {
      success: false,
      message,
      table_count: 0,
      ...
      query_context: context2
    };
  }
  createSqlError(query, error3, suggestions) {
    return {
      success: false,
      error: error3,
  ...
  generateSqlErrorSuggestions(query, error3) {
    const suggestions = [];
    const errorMessage = error3 instanceof Error ? error3.message.toLowerCase() : String(error3).toLowerCase();
  ...
  generateExecutionHints(query, resultCount) {
    const hints = [];
  ...
  generatePerformanceNotes(query, resultCount) {
    const notes = [];
    const lowerQuery = query.toLowerCase();
    if (resultCount > 1e3) {
  ...
  generateProcessingGuidance(schemas) {
  ...
  generateFieldSuggestions(schemas) {
    const suggestions = {};
    for (const [tableName, schema] of Object.entries(schemas)) {
  ...
  validateAnalyticalSql(sql) {
    ...
    const allowedStarters = [
      ...
      "create temp view",
      "drop view",
      // Clean up session views
      "drop temporary table",
      "drop temp table"
    ];
    const blockedPatterns = [
      /\bdrop\s+table\s+(?!temp|temporary)/i,
      // Block permanent table drops
      /\bdelete\s+from/i,
      // Block data deletion
      /\bupdate\s+\w+\s+set/i,
      // Block data updates
      /\binsert\s+into\s+(?!temp|temporary)/i,
      // Block permanent inserts
      /\balter\s+table/i,
      // Block schema changes
      /\bcreate\s+table\s+(?!temp|temporary)/i,
      // Block permanent table creation
      /\battach\s+database/i,
      // Block external database access
      /\bdetach\s+database/i
      // Block database detachment
    ];
    const startsWithAllowed = allowedStarters.some(
      (starter) => trimmedSql.startsWith(starter)
    );
  ...
  async createTablesWithValidation(schemas) {
  ...
  validateAndFixIdentifier(name, type) {
    if (!name || typeof name !== "string") {
      return type === "table" ? "fallback_table" : "fallback_column";
    }
    let fixed = name.replace(/[^a-zA-Z0-9_]/g, "_").replace(/_{2,}/g, "_").replace(/^_|_$/g, "");
    if (/^[0-9]/.test(fixed)) {
  ...
  validateSQLiteType(type) {
    ...
    const validTypes = [
      ...
      "JSON"
      // SQLite supports JSON since 3.38
    ];
  ...
  async generateEnhancedMetadata(schemas, sourceQuery) {
    const metadata = {
      schemas: {},
      total_rows: 0,
      query_guidance: {
  ...
  async getSchemaInfo() {
    try {
      ...
      for (const table3 of tables) {
        const tableName = String(table3.name);
      ...
      return {
        success: false,
        error: error3 instanceof Error ? error3.message : "Schema inspection failed",
        suggestions: [
  ...
  async getTableColumns(tableName) {
    try {
      ...
      return {
        ...
        table: tableName,
        columns: columns.map((col) => {
          const fkRef = foreignKeys.find((fk) => fk.from === col.name);
          return {
            name: col.name,
            type: col.type,
            not_null: Boolean(col.notnull),
      ...
      return {
        success: false,
        error: error3 instanceof Error ? error3.message : "Table inspection failed",
        suggestions: [
  ...
  async generateAnalyticalQueries(tableName) {
    try {
      ...
      return {
        success: false,
        error: error3 instanceof Error ? error3.message : "Query generation failed",
  ...
  async generateContextualSuggestions(tableName) {
    if (!tableName) {
      return {
        getting_started: {
    ...
    try {
      ...
      if (columns.some((col) => String(col.name).toLowerCase().includes("id"))) {
  ...
  async concatenateStrings(tableName, columns, separator = " ", whereClause, rowLimit) {
    try {
      ...
      const concatenated = rows.map((row) => {
      ...
      return {
        success: false,
        error: error3 instanceof Error ? error3.message : "String concatenation failed",
        suggestions: [
          `Verify table name: ${tableName}`,
  ...
  async fetch(request) {
    ...
    try {
      if (url.pathname === "/process" && request.method === "POST") {
        ...
        const result = await this.generateAnalyticalQueries(tableName || void 0);
        return new Response(JSON.stringify(result), {
          headers: { "Content-Type": "application/json" }
        });
        ...
        const { table_name, columns, separator, where_clause, row_limit } = await request.json();
...
var datasetRegistry = /* @__PURE__ */ new Map();
var RcsbPdbMCP = class extends McpAgent {
  constructor() {
    super(...arguments);
...
Example data query: '{ entry(entry_id:"4HHB") { struct { title } exptl { method } } }'.`),
      ...
      async ({ query, variables }) => {
        try {
          ...
          return this.createErrorResponse("GraphQL execution failed", error3);
        }
      }
    );
    this.server.tool(
      ...
      async ({ data_access_id, sql }) => {
        try {
          ...
          return this.createErrorResponse("SQL execution failed", error3);
        }
      }
    );
    this.server.tool(
      ...
      async ({ data_access_id, table_name, columns, separator, where_clause, row_limit }) => {
        try {
          ...
          return this.createErrorResponse("String concatenation failed", error3);
        }
      }
    );
  }
  ...
  // ========================================
  async executeRcsbPdbGraphQLQuery(query, variables) {
    try {
      const headers = {
          ...
          errors: [
            {
              ...
              extensions: {
                statusCode: response.status,
                responseText: errorText.slice(0, 1e3)
              }
            }
          ]
        };
  ...
  isIntrospectionQuery(query) {
    ...
    const normalizedQuery = query.replace(/\s*#.*$/gm, "").replace(/\s+/g, " ").trim().toLowerCase();
    const introspectionPatterns = [
      ...
      "introspectionquery",
      // Named introspection queries
      "getintrospectionquery"
    ];
    return introspectionPatterns.some(
      (pattern) => normalizedQuery.includes(pattern)
    );
  }
  shouldBypassStaging(result, originalQuery) {
    ...
    if (result.data) {
      ...
      const hasSchemaMetadata = Object.values(result.data).some((value) => {
        if (value && typeof value === "object") {
          ...
          return schemaFields.some((field) => keys.includes(field)) || typeFields.filter((field) => keys.includes(field)).length >= 2;
        }
        return false;
      });
  ...
  detectMultipleEntities(data) {
    if (!data || typeof data !== "object") return false;
    for (const value of Object.values(data)) {
      if (Array.isArray(value) && value.length > 1) {
        const firstItem = value[0];
        if (firstItem && typeof firstItem === "object" && (firstItem.id || firstItem.rcsb_id || Object.keys(firstItem).length >= 3)) {
          return true;
        }
      }
    }
  ...
  detectComplexNesting(data, depth = 0) {
  ...
  // ========================================
  async stageDataInDurableObject(graphqlResult) {
    const env2 = this.env;
    if (!env2?.JSON_TO_SQL_DO) {
      throw new Error("JSON_TO_SQL_DO binding not available");
    }
    const accessId = crypto.randomUUID();
    const doId = env2.JSON_TO_SQL_DO.idFromName(accessId);
    const stub = env2.JSON_TO_SQL_DO.get(doId);
  ...
  async executeSQLQuery(dataAccessId, sql) {
    const env2 = this.env;
    if (!env2?.JSON_TO_SQL_DO) {
      throw new Error("JSON_TO_SQL_DO binding not available");
    }
    const doId = env2.JSON_TO_SQL_DO.idFromName(dataAccessId);
    const stub = env2.JSON_TO_SQL_DO.get(doId);
  ...
  async concatenateStrings(dataAccessId, tableName, columns, separator, whereClause, rowLimit) {
    const env2 = this.env;
    if (!env2?.JSON_TO_SQL_DO) {
      throw new Error("JSON_TO_SQL_DO binding not available");
    }
  ...
  async deleteDataset(dataAccessId) {
    const env2 = this.env;
    if (!env2?.JSON_TO_SQL_DO) {
      throw new Error("JSON_TO_SQL_DO binding not available");
    }
  ...
  // ========================================
  createErrorResponse(message, error3) {
    return {
      content: [{
        type: "text",
        text: JSON.stringify({
...
var src_default = {
  async fetch(request, env2, ctx) {
    ...
    if (url.pathname.startsWith("/datasets/") && request.method === "DELETE") {
      ...
      const doId = env2.JSON_TO_SQL_DO.idFromName(id);
      const stub = env2.JSON_TO_SQL_DO.get(doId);
...
// .wrangler/tmp/bundle-dW0Zly/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-dW0Zly/middleware-loader.entry.ts
